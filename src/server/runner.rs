use bevy::{
    ...
    utils::{Duration, Instant}, log::debug, prelude::*
};
...
#[derive(Default)]
pub struct ServerRunnerPlugin {
    /// Determines whether the [`Schedule`](bevy_ecs::schedule::Schedule) is run once or repeatedly.
    pub run_mode: RunMode,
}

impl ServerRunnerPlugin {
    /// See [`RunMode::Once`].
...
impl Plugin for ServerRunnerPlugin {
                ...
                RunMode::Loop { wait } => {
                    let mut tick = move |app: &mut App,
                        ...
                        if let Some(wait) = wait {
                            ...
                            if exe_time < wait {
                                //debug!("Wait time: {:?}, Execution Time: {:?}", wait, exe_time);
                    ...
                    #[cfg(not(target_arch = "wasm32"))]
                    {
                        while let Ok(delay) = tick(&mut app, wait) {
                            if let Some(delay) = delay {
                                //debug!("Theoretical Delay: {:?}", delay);
                                //let start_time = Instant::now();
                                //std::thread::sleep(delay);
                                spin_sleep::sleep(delay);
                                //let end_time = Instant::now();
                                //debug!("Actual Delay: {:?}", end_time - start_time);
                            }
                        }
                    }
