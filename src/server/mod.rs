...
use steamworks::{
    Server, ServerManager, SingleClient, SteamServerConnectFailure, SteamServersConnected,
    ...
    fn build(&self, app: &mut App) {
        ...
        let steam_server = match Server::init(
            ...
            Err(error) => {
                ...
                msgbox::create(
                    "Error initializing Steam Server!",
                    format!("{}", error).as_str(),
        ...
        steam_server
            .0
            .register_callback(|failure: SteamServerConnectFailure| {
                error!(
                    "Failed connecting to Steam servers \"{}\". {}",
                    failure.reason,
                    if failure.still_retrying {
                        "We'll try again."
                    } else {
        ...
        steam_server
            .0
            .register_callback(|_: SteamServersConnected| {
                info!("Connected to Steam servers!");
            });

        steam_server
            .0
            .register_callback(|disconnected: SteamServersDisconnected| {
                error!(
                    "We have been disconnected from the Steam servers \"{}\"",
                    disconnected.reason
                );
            });
